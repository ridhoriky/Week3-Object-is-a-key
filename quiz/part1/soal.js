...
function fpb(angka1, angka2) {
  // you can only write your code here!
  const angkaKecil = Math.min(angka1, angka2);
  let hasil = 1;
  for (let i = 2; i <= angkaKecil; i++) {
...
function cariMedian(arr) {
  ...
  let hasil = arr[Math.floor(panjang / 2)];
  if (panjang % 2 === 0) {
    const setengah = panjang / 2;
    hasil = (arr[setengah - 1] + arr[setengah]) / 2;
  }
...
function cariModus(arr) {
  ...
  if (max === 1 || Object.keys(modus).length === 1) return -1;
...
function ubahHuruf(kata) {
  ...
  return kata.replace(/[a-z]/g, (letter) => nextLetter(letter));
}
