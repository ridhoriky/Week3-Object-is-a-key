...
function countProfit(shoppers) {
  ...
  const result = listBarang.map((item, index) => {
  ...
  for (let i = 0; i < shoppers.length; i++) {
    ...
    const index = productMap[product];

    if (index === undefined) continue;

    if (result[index].leftOver >= amount) {
      result[index].shoppers.push(name);
      result[index].leftOver -= amount;
      result[index].totalProfit += amount * listBarang[index][1];
    }
  }
