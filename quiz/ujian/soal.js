...
function naikAngkot(arrPenumpang) {
  rute = ["A", "B", "C", "D", "E", "F"];
  //your code here
  const result = arrPenumpang.map((item) => {
    const [penumpang, naikDari, tujuan] = item;
    const jarakTempuh = rute.indexOf(tujuan) - rute.indexOf(naikDari);
    const bayar = jarakTempuh * 2000;
    return {
      penumpang,
      naikDari,
      tujuan,
...
function graduates(students) {
  ...
  for (const student of students) {
    ...
    if (!result[className] && student.score > 75) {
      result[className] = [
        {
          name: student.name,
          score: student.score,
